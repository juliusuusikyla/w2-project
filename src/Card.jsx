...
export default function Card({ id, hp, attack, name, type, typeColor }) {
  return (
    <div
      className="card"
      style={{ borderColor: typeColor }}
      onMouseEnter={(e) => (e.currentTarget.style.borderWidth = "10px")}
      onMouseLeave={(e) => (e.currentTarget.style.borderWidth = "")}
    >
      ...
      <div className="beast-img">
        <img
          src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${id}.png`}
        />
      </div>
      <div className="beast-name">{name}</div>
      <div className="beast-type">
        <span className="type-dot" style={{ background: typeColor }}></span>
        {type}
      </div>
    </div>
  );
